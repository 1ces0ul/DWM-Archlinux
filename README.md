...
archlinux安装完成后设置neovim为系统默认编辑器不然git等场景下终端调用编辑器会失效，在`/etc/profile.d/editor.sh` 中写入 `export EDITOR='nvim'`然后`chmod a+x editor.sh` ， 因为 `/etc/profile`会source`profile.d`中的文件
...
dwm patches:
1. alpha-systray-6.3full
2. hidevacanttags
3. noborder
4. decorhints (有些软件如pwvucontrol会生成奇怪的边框，需要加上修改picom和gtk.css来解决)
